...
export default function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const result = await register(username.trim(), password, qqNumber.trim());
    setLoading(false);
    if (!result.success) {
      setError(result.message);
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-primary/10 border border-primary/30 glow-cyan mb-4">
            <DiamondLogo size={40} className="text-primary" />
          </div>
          <h1 className="text-3xl font-bold text-glow-cyan mb-2">石棺</h1>
          <p className="text-muted-foreground text-sm">明日方舟解谜群 · 非公开博客</p>
        </div>
